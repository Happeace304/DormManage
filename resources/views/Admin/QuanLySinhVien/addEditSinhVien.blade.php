                ...
                <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" method="post" action="{{$action}}" enctype="multipart/form-data">
                   ...
                   @if(isset($user)) @method('PUT') @endif
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input type="text" id="email" name="email" {{isset($user)?'readonly':''}} class="form-control col-md-7 col-xs-12"
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select class="form-control" name="roomId" id="roomId" tabindex="2">
                                @foreach($room as $item)
                                    <option @if(isset($user)){{$user->roomId == $item->roomId?'selected':''}} @endif value="{{$item->roomId}}">{{$item->roomName}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
